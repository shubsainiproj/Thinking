...
__device__ __forceinline__ void RIPEMD160_from_SHA256_state(const uint32_t sha_state_be[8], uint8_t ripemd20[20]) {
    uint32_t W[16];
#pragma unroll
...
__device__ __noinline__ void getHash160_33_from_limbs(uint8_t prefix02_03, const uint64_t x_be_limbs[4], uint8_t out20[20]) {
    uint32_t sha_state[8];
    SHA256_33_from_limbs(prefix02_03, x_be_limbs, sha_state);
    RIPEMD160_from_SHA256_state(sha_state, out20);
}

// Host implementation of getHash160_33_from_limbs
__host__ void getHash160_33_from_limbs_host(uint8_t prefix02_03, const uint64_t x_be_limbs[4], uint8_t out20[20]) {
    // Construct 33-byte public key
    uint8_t pubkey[33];
    pubkey[0] = prefix02_03;
    int off = 1;
    for (int limb = 3; limb >= 0; --limb) {
        uint64_t v = x_be_limbs[limb];
        pubkey[off + 0] = (uint8_t)(v >> 56);
        pubkey[off + 1] = (uint8_t)(v >> 48);
        pubkey[off + 2] = (uint8_t)(v >> 40);
        pubkey[off + 3] = (uint8_t)(v >> 32);
        pubkey[off + 4] = (uint8_t)(v >> 24);
        pubkey[off + 5] = (uint8_t)(v >> 16);
        pubkey[off + 6] = (uint8_t)(v >> 8);
        pubkey[off + 7] = (uint8_t)(v);
        off += 8;
    }

    // Compute SHA256
    uint8_t sha256[32];
    host_sha256::sha256(pubkey, 33, sha256);
    ...
    uint32_t s[5] = {0x67452301ul, 0xEFCDAB89ul, 0x98BADCFEul, 0x10325476ul, 0xC3D2E1F0ul};
    uint32_t u;
    uint32_t a1 = s[0], b1 = s[1], c1 = s[2], d1 = s[3], e1 = s[4];
    uint32_t a2 = a1, b2 = b1, c2 = c1, d2 = d1, e2 = e1;
    ...
    for (int i = 0; i < 5; ++i) {
        out20[4 * i + 0] = (uint8_t)(s[i] >> 0);
        out20[4 * i + 1] = (uint8_t)(s[i] >> 8);
        out20[4 * i + 2] = (uint8_t)(s[i] >> 16);
        out20[4 * i + 3] = (uint8_t)(s[i] >> 24);
    }
}
